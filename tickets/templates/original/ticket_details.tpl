...
{% if not primary.waiting %}
	...
	<li class="{% if primary.paid %}current{% else %}complete{% endif %}">
		<h5>2</h5>
		<span>
			Payment
			{% if primary.paid %}
	...
	<li class="{% if primary.collected %}current{% elif not primary.paid %}future{% else %}complete{% endif %}">
		<h5>3</h5>
		<span>
			Collection
	...
	<li>
	<li class="{% if primary.checked_in %}current{% elif not primary.collected %}future{% else %}complete{% endif %}">
		<h5>4</h5>
		<span>
			Entry
...
{% if forloop.counter0 == 0 %}
			<td>Not permitted</td>
{% else %}
			<td><a href="{# url 'namechange'#}">Request a change</a></td>
{% endif %}
{% endif %}
		</tr>
{% endfor %}
...
<p>In order to complete your name change, you will need to send a Bank Transfer to <strong>King&rsquo;s Affair</strong> for the amount of <strong>&pound;{{ primary.price }}</strong>.</p>
